...
int instruction_live(struct s_cpu *cpu, struct s_process *proc)
{
  ...
  arg1 = (int)read_mem_4(cpu->program, proc->pc + 1);
...
int instruction_ld(struct s_cpu *cpu, struct s_process *proc)
{
  ...
  if (check_pcb((pcb = read_mem_1(cpu->program, proc->pc + 1)), e_ld) != 0)
  {
    if ((type = type_from_pcb(pcb, 0)) == T_DIR)
      arg1 = read_mem_4(cpu->program, proc->pc + 2);
    else
      arg1 = read_indirect(cpu, proc, read_mem_2(cpu->program, proc->pc + 2));
    arg2 = read_mem_1(cpu->program, proc->pc + 2 + size_from_pt(type, e_ld));
    if (valid_reg(arg2) != 0)
    {
  ...
  ret = proc->pc + 2 + size_from_pcb(pcb, e_ld);
  if (f_verbose & OPT_PCMOVE)
  ...
  if (check_pcb(pcb, e_st) != 0)
  {
    ...
    if (valid_reg(arg1) != 0)
    {
      ...
      if ((type == T_REG) && valid_reg(arg2) != 0)
        ...
        write_mem_ins(proc, cpu->program, mod_idx(proc->pc + arg2 % IDX_MOD), read_reg(proc, arg1));
    }
  }
  if (f_verbose & OPT_PCMOVE)
