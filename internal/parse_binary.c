...
unsigned next_instruction(char *linebuf, size_t bufsize, size_t *bufidx,
                          uint8_t *progbuf, size_t progbuf_size, int *err) {
  ...
  *bufidx += g_op_len_tab[opcode - 1];
  for (int jj = 0;
       *bufidx < bufsize && jj < g_op_tab[opcode].numargs && ii < progbuf_size;
       ++jj) {
...
int parse_bin(char *buf, size_t bufsize, int fd, size_t *size) {
  ...
  for (ii = 0; ii < progbuf_size;) {
    ii += next_instruction(buf, bufsize - 10, &bufidx, progbuf + ii,
                           progbuf_size - ii, &err);
    if (err) {
      free(progbuf);
      progbuf = NULL;
