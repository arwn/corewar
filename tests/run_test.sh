...
if [[ $input_file != "" ]]; then
    ...
    $outvm -r -d "$tim" -f "$out/$bn.cor" > /tmp/out
...
for ins in "${input_args[@]}"; do
    ...
    for i in $(seq 1 "$j"); do
        ...
        if [[ $experimental -eq 1 ]]; then
            $outvm -r -v 1 -d "$tim" -f "$out/$ins$i.cor" | awk '{$1=$1};1' > "$out/$ins$i.out"
        else
            timeout 2 $outvm -r -d "$tim" -f "$out/$ins$i.cor" > "$out/out.tmp"
            status=$?
            if (( status!=124 )); then
